...
class BookingServiceTest {
    ...
    void testCreateBooking() {
        ...
        Flight flight = new Flight(departureAirport, arrivalAirport, LocalDateTime.now(), FlightStatus.AVAILABLE,
                200.0f, 100, 150);
      ...
      Long bookingId = 1L;
      Booking existingBooking = booking; // existing booking mock
      BookingDto updatedBookingDto = bookingDto; // updated DTO
      
      when(bookingRepository.findById(bookingId)).thenReturn(Optional.of(
     existingBooking));
      when(userRepository.findByUsername(updatedBookingDto.user().username())).
  ...
  assertNotNull(result);
   ...
   @Test
    void testDeleteBooking() {
        ...
        200.0f, 100, 150);
        User user = new User(1L, "john"); 
        Booking booking = new Booking(user, LocalDateTime.now(), 2, flight); // booking mock 
        ...
        verify(flightRepository).save(flight); // Verify flight's available seats are updated
    }
}
